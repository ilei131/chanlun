...
### 4.5 接口安全规范
- **请求方式**：接口优先使用 POST 请求，确保请求体可以携带完整的数据和 token
- **Token 校验**：除登录、注册等公开接口外，所有其他接口必须进行 JWT Token 有效性校验
- **例外情况**：以下接口可使用 GET 请求且无需 Token（需明确标记）：
  - 公开的股票列表查询（无敏感数据）
  - 公开的搜索接口（无敏感数据）
  - 静态资源获取
- **错误处理**：Token 无效或过期时，返回 401 Unauthorized 状态码，并给出清晰的错误提示
