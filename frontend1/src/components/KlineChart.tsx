...
export default function KlineChart({ kline, ma, buyPoints, sellPoints, zsList }: KlineChartProps) {
  ...
  useEffect(() => {
    ...
    // Sort kline data by date ascending (lightweight-charts requirement)
    const sortedKline = [...kline].sort((a, b) => a.date.localeCompare(b.date));

    // Candlestick series - A-share colors: red for up, green for down
    ...
    // Pivot (Zhongshu) zones - use line series to show upper and lower bounds
    zsList.forEach((zs) => {
      ...
      // Find kline data between start and end dates
      ...
      if (startIdx >= 0 && endIdx >= 0) {
        ...
        if (slice.length > 0) {
          upperLine.setData(slice.map((k) => ({ time: k.date as Time, value: zs.zg })));
          lowerLine.setData(slice.map((k) => ({ time: k.date as Time, value: zs.zd })));
        }
      }
    });
  ...
  }, [kline, ma, buyPoints, sellPoints, zsList]);
